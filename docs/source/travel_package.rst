...
.. code-block:: bash
    ...
    curl -X 'POST' \
      'https://api.example.com/policy-package/traveling-insurance-package' \
...
در صورت موفقیت، یک شناسه پکیج ایجاد می‌شود که در ادامه برای دریافت جزئیات، بروزرسانی یا حذف استفاده خواهد شد.
...
``GET``
``/policy-package/traveling-insurance-package/{traveling_insurance_package_id}``
...
این سرویس امکان مدیریت کامل چرخه عمر پکیج بیمه‌نامه‌های مسافرتی را فراهم می‌کند:

- ایجاد پکیج و شروع صدور آنلاین به‌صورت غیرهمزمان
- مشاهده وضعیت و جزئیات پکیج
- ویرایش اطلاعات قبل از صدور
- حذف پکیج در صورت عدم صدور بیمه‌نامه
